...
::: callout-tip
Given that DfE policy is to use dashboards as supplementary tools to Official Statistics releases on EES, dashboards themselves should *not* display the Official Statistics logo. This should be reserved purely for the primary release of the data via EES.
:::
...
Dashboards, along with the Official Statistics that they supplement, are a digital service that we provide, and we should ensure we follow the [Service Standard](https://www.gov.uk/service-manual/service-standard), and the [Office for Statistics Regulation (OSR) regulatory guidance on dashboards](https://osr.statisticsauthority.gov.uk/publication/regulatory-guidance-dashboards/).

The standards and regulatory guidance cover a number of aspects that all producers should be considering when developing a dashboard, and strongly influence the guidance on this page.
