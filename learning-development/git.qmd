...
![](../images/git_command_diagram.svg){fig-alt="Diagram illustrating four Git file locations: working directory (current folder), staging area (files prepared for commit), local repository (version-controlled code on your machine), and remote repository (version-controlled code on a server). Arrows show commands: git clone copies from remote to local; git checkout moves code from local to staging and working directory; git add moves files from working directory to staging; git commit moves from staging to local; git push sends to remote; git pull retrieves from remote to local, staging, and working directory, combining git fetch and git merge."}
