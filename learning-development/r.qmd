...
Tips for reaching best practice in R can be found on our [RAP page](../RAP/rap.html#Clean_final_code), with guidance on meeting best practice in RAP for clean final code. This makes it easier to read and pick up if another person is running your code. 
...
R can be used to execute SQL scripts to extract data from a database as well as querying the database directly via R. There are two primary ways to do this: 1. executing a SQL script as is within R and 2. using dbplyr to create SQL code for you. Which you use will depend on how comfortable with SQL and R and also if you already have existing SQL scripts that you want to execute or you're writing new database queries.
...
As with sending a SQL script, you'll need to create the connection to the database first:
...
Once you've established a connection, you can then create piped queries as you would with dplyr:
...
The key differences to just using dplyr are:
...
This function can also be found in the [dfeR R package](https://github.com/dfe-analytical-services/dfeR){target="_blank" rel="noopener noreferrer"}.
