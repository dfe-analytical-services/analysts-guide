...
The policy of creating tidy data files effectively means optimising your filter-indicator combinations for use within the EES user interface. By doing so, end users will be better able to interact with your data and find the information that they're looking for. Tidy data structures work better in the EES table tool when compared to wide (or pivoted) data sets.
...
If your current processes produce wide data that you need to switch to a tidy structure, one of doing this is using the `pivot_longer()` function in R. 
