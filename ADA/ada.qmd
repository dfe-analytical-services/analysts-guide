...
The ADA project is currently in its onboarding phase, which is expected to continue until 2025. At the moment, teams can onboard voluntarily until **???**. Following this date, the remaining teams will have their data migrated for them. 
...
The ADA team also intends to establish an analyst Community of Practice that:
...
A list of current ADA champions can be found on the [ADA intranet page](https://educationgovuk.sharepoint.com/sites/lvewp00505), they are able to provide advice and support if you run into any issues. If you wish to become an ADA champion then please contact the [ADA team](mailto:ADAPTTeam@education.gov.uk).
...
Notebooks might be something you've not come across before. Notebooks are made up of multiple code or markdown cells, which may mean that you have to structure your code in a different way than you're used to. They are different to IDEs (Integrated Development Environments) like RStudio. You can find more information on Databricks notebooks and their structure on the [ADA guidance site](https://analytical-data-access.education.gov.uk/get-support/using-r-python-and-sql-in-databricks).
...
Due to reasons highlighted above, for regular analysis pipelines adopting RAP principles we'd usually advise against using notebooks and instead recommend a Git controlled repository of code scripts using the [standard repository template available in dfeR](https://dfe-analytical-services.github.io/dfeR/reference/create_project.html). In addition to potential problems raised due to unfamiliarity with notebooks themselves, version control can also be more difficult in Databricks notebooks, as there is the potential for multiple people to make edits to the same notebook at once. Additionally, with a full script in an IDE, when the code is run without modifications it will provide the same output consistently. However, with notebooks you can run code cells independently of one another. If you don't run all code cells in order then your output could be inconsistent and result in errors, potentially making QA more difficult. This introduces more manual steps and increases the risk of human error. 
...
You can write new SQL code in the Databricks SQL Editor, which is very similar to SSMS. 
...
If you'd like to make use of automated workflows to run scheduled code, then your SQL code will need to be moved into Databricks notebooks, using Spark SQL syntax. 
...
Your R code can remain in RStudio and all existing R processes can still be run there. Code that points towards SQL scripts will need to be redirected to Databricks rather than SSMS. In order to use RStudio with a Databricks SQL warehouse, you will need to manually set up a connection. You can find more information about that on our [setup guidance page](/ADA/databricks_rstudio_sql_warehouse.html). 
