...
-   Installing the `pysparklyr` and `sparklyr` packages
...
This may take a few minutes and once completed, you should see your catalogs and schemas in the RStudio Connections pane.

You can test the connection by entering the below code into the console and updating `[catalog name]` with the name of the catalog you want to use.

``` r
sdf_sql(sc, "USE CATALOG "[catalog name];")
```

::: callout-important
If you are re-installing sparklyr after previously connecting with the reticulate method, you may need to first delete reticulate. You can do this by:

1.  Press the Windows key + R and an app will open.
2.  Type %appdata% in the text field called 'Open'.
3.  In the File Explorer window that opens, navigate up one level to the AppData folder.
4.  Open the Local folder.
5.  Inside Local, go to the RStudio folder and delete the reticulate folder.
6.  Return to the Local folder, then go to R \> cache, and delete the reticulate folder there as well.```
2.  In the File Explorer window that opens, navigate up one level to the AppData folder.
3.  Open the Local folder.
4.  Inside Local, go to the RStudio folder and delete the reticulate folder.
5.  Return to the Local folder, then go to R \> cache, and delete the reticulate folder there as well.
:::
