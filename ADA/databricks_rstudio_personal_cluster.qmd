...
Before following the process, ensure you are on the [correct Databricks Workspace](../ADA/databricks_fundamentals.html#workspaces-dropdown-guide) by clicking on the Workspace drop-down, usually found in the top right of the Databricks interface. Make sure you are in the production Workspace by checking that you are on the Workspace that begins `s101p01`  — the **`p`** indicates production.
...
3. Open the 'Advanced' drop-down and ensure Access mode is set to 'Manual' and choose 'Dedicated'. 

4.  Click the 'Create compute' button at the bottom of the page. This will create your personal cluster and begin starting it up. This usually takes around 5 minutes\
    \
    ![](../images/ada-compute-personal-create-button.png)

5.  Once the cluster is up and running the icon under the 'State' header on the 'Compute' page will appear as a green tick\
    \
...
![](../images/databricks-create-token.png)

- Give the token a sensible name so that you can identify it in your list of tokens.
- Increase the Lifetime to something reasonable (90 days is industry standard), every time the token expires, you will have to follow this guidance again to set up a new token and reconnect to Git, so choose something sensible. 
- You should make sure that "all-apis" is selected under the API scope(s) heading. 

When you have entered all of the required information, click "Generate" at the bottom of the window. 
...
```
::: callout-note
Databricks have recently [renamed their ODBC driver](https://docs.databricks.com/aws/en/integrations/odbc/migration) which may cause issues with the `odbc` R package while it catches up. We've added a line to the code above to specify the driver, which provides a fix in the meantime.
:::
...
In the case of this error, check that your personal compute has been set up correctly. Click on the advanced drop-down within your personal compute configuration page and ensure that 'Access Mode' is set to 'Manual' and select 'Dedicated'. 

::: {align="center"}
![](../images/ada-compute-personal-advanced.png)
:::
