...
There are a few different ways of storing files and data on DataBricks.
...
The unity catalog can be accessed through the 'Compute' option in the DataBricks sidebar.
...
```         
  ○ To actually run code you'll need to use a 'Compute resource'
      § Essential a computer (or group of computers) in the cloud
  ○ Types of compute
      § SQL Warehouses
          □ Can only run SQL, but are optimised for SQL queries
      § Personal cluster
          □ Can run R, python, SQL or scala
      § Shared Clusters
          □ Can be shared by multiple users
          □ Currently doesn't support R, so won't focus on this for now
              ® Support is planned for the future but not got a specific date yet
  ○ How do you set them up
      § SQL Warehouses
          □ Will be set up centrally by the ADA team
      § Personal cluster
          □ You can set up yourself under certain policies
          □ You can also ask for a larger cluster if your work requires it, but this will be set up by the ADA team
          □ Compute
              ® Create with DfE Personal Compute
                  ◊ Databricks Runtime
                  ◊ Node Type
              ® Using compute
                  ◊ You'll need to start the compute before running code
                      } Can be done 
                          – From the compute page
                          – From a notebook 
                          – From the SQL editor
                  ◊ Will take a few minutes to start up
                  ◊ If unused for 1 hour it will shut down again to preserve cost to the Dept
```
