...
If you wish to continue using `sqlQuery()` in your code, you will need to set up your connection using `RODBC` instead of `odbc`. You can find guidance on how to do this [on the Databricks guidance website](https://learn.microsoft.com/en-us/azure/databricks/integrations/odbc/connect-databricks-excel-python-r).

The other alternative is to continue using `odbc` and switch to the `DBI` package instead, as in this example:

``` {r dbgetquery, eval=FALSE}
...
# dbExecute can be used for more complex data manipulation
